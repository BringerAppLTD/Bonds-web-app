import {Link} from 'react-router-dom'

export default function MyDate(date) {
    return (
        <>
        <Link to={"/formabonditem"} style={{textDecoration: 'none'}}>
                <div className="formAbond-element" style={{lineHeight: 0.9, borderRadius: 12, color: 'black', cursor: 'pointer'}}>
                    <img src={date.songPic} className="formABondImage"/>

                    <div className="formABondImageContent">
                            <div style={{display:'flex', justifyContent: 'end'}}>
                                        <p style={{fontSize: 14, paddingRight: 4, color: 'white'}}><b>{date.timeStarting}</b></p>
                                <Link to="/profile" style={{textDecoration: 'none'}}>      
                                        <img src={date.ownerPic} style={{borderRadius: 19, border: '2px solid limegreen', width: 38, height: 38, marginTop: 16, marginRight:12 }}/>
                                </Link>
                            </div>
                            <p style={{fontSize: 14, color: 'white'}}>{date.artistName}: <br/><b>{date.songTitle}</b></p>
                    </div>
                </div>
        </Link>
        </>
    )
}